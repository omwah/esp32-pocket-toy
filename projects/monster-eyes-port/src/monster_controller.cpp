...
bool MonsterController::refreshPackages() {
    ...
    for (File entry = root.openNextFile(); entry; entry = root.openNextFile()) {
        ...
        for (size_t i = 0; i < name.length(); ++i) {
            if (name[i] == '_' || name[i] == '-') {
                ...
                name.setCharAt(i, toupper(name[i]));
                capitalize = false;
            }
        }
        _packages.push_back({id, name, config, true});
    }
    std::sort(_packages.begin(), _packages.end(),
    ...
    while (start < savedOrder.length()) {
        int end = savedOrder.indexOf(',', start);
        if (end < 0) end = savedOrder.length();
    ...
    for (const auto &p : _packages) {
        if (reconciled.length()) reconciled += ',';
        reconciled += p.id;
    }
    if (reconciled != savedOrder) {
        orderPrefs.begin("monster-eyes", false);
        orderPrefs.putString("order", reconciled);
        orderPrefs.end();
    }
...
bool MonsterController::begin() {
    ...
    for (uint8_t i = 0; i < _packages.size(); ++i) {
        _packages[i].enabled =
            prefs.getBool(enabledKey(_packages[i].id).c_str(), true);
...
bool MonsterController::navigate(int direction) {
    ...
    for (size_t attempts = 0; attempts < _packages.size(); ++attempts) {
        candidate =
            (candidate + direction + _packages.size()) % _packages.size();
...
bool MonsterController::setPackageOrder(const String &csv) {
    ...
    while (start < csv.length()) {
        ...
        String id = csv.substring(start, end);
        int idx = packageIndex(id);
        if (idx < 0) return false;
        for (const auto &p : ordered)
            if (p.id == id) return false;
        ordered.push_back(_packages[idx]);
        start = end + 1;
    }
    if (ordered.size() != _packages.size()) return false;
    String current = _packages[_style].id;
    _packages.swap(ordered);
    _style = packageIndex(current);
    Preferences prefs;
    prefs.begin("monster-eyes", false);
    prefs.putString("order", csv);
    prefs.end();
    return true;
}

bool MonsterController::reloadPackages(const String &preferredId) {
    String id = preferredId.length()
                    ? preferredId
                    : (_packages.empty() ? String() : _packages[_style].id);
    if (!refreshPackages()) return false;
    Preferences prefs;
    prefs.begin("monster-eyes", true);
    for (auto &p : _packages)
        p.enabled = prefs.getBool(enabledKey(p.id).c_str(), true);
    prefs.end();
    int idx = packageIndex(id);
    if (idx < 0) idx = 0;
...
bool MonsterController::setStyleEnabled(uint8_t style, bool enabled) {
    if (style >= _packages.size()) return false;
    if (!enabled && _packages[style].enabled && enabledStyleCount() == 1)
        return false;
